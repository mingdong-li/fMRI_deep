        ...
        for phase in ['train', 'val']:
            ...
            for i, data in enumerate(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase=='train'):
                    ...
                    _, preds4 = torch.max(id0_list[3], 1)
                    _, preds3 = torch.max(id0_list[2], 1)
                    _, preds2 = torch.max(id0_list[1], 1)
                    _, preds1 = torch.max(id0_list[0], 1)
                    running_correct4 = (preds4 == img0_label).sum().item()
                    running_correct3 = (preds3 == img0_label).sum().item()
                    running_correct2 = (preds2 == img0_label).sum().item()
                    running_correct1 = (preds1 == img0_label).sum().item()
                    
                    print(" Current acc {}\n".format(float(running_correct4 + 
                        running_correct3 + running_correct2 + running_correct1)
                        /(4*len(img0_label))))
                ...
                if phase == 'val':
                    ...
                    labels = labels.cuda()
                    out = model(img0,img1)
                    _, preds4_v = torch.max(out['id0'][3], 1)
                    _, preds3_v = torch.max(out['id0'][2], 1)
                    _, preds2_v = torch.max(out['id0'][1], 1)
                    _, preds1_v = torch.max(out['id0'][0], 1)
                    val_correct4 += (preds4_v == labels).sum().item()
                    val_correct3 += (preds3_v == labels).sum().item()
                    val_correct2 += (preds2_v == labels).sum().item()
                    val_correct1 += (preds1_v == labels).sum().item()
            ...
            if phase == 'val':
                epoch_acc4 = float(val_correct4)/len(fmri_datasets[phase])
                epoch_acc3 = float(val_correct3)/len(fmri_datasets[phase])
                epoch_acc2 = float(val_correct2)/len(fmri_datasets[phase])
                epoch_acc1 = float(val_correct1)/len(fmri_datasets[phase])
                acc_list = [epoch_acc1,epoch_acc2,epoch_acc3,epoch_acc4]
                counter.append(epoch)
                for k, key in enumerate(['val1','val2','val3','val4']):
                    accuracy[key].append(acc_list[k])

                epoch_acc = sum(acc_list)/4
                accuracy['avg_acc'].append(epoch_acc)
