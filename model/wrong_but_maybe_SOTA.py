...
class Res2Net(nn.Module):
    # 可以筛选后卷积层load MedicalNet，res2net和res2net只是forward和内部细化结构不同吧？
    def __init__(self, block, layers, baseWidth = 26, scale = 4, num_classes=1000):
        ...
        # 这里的fc层貌似是错的，dropout和relu的关系怎么放
        # 但是结果,貌似是SOTA的
        self.fc1 = nn.Sequential(
            nn.Linear(64 * block.expansion, num_classes*4),
            nn.Dropout(0.5),
            nn.ReLU(inplace=True),
            nn.Linear(num_classes*4, num_classes),
            nn.ReLU(inplace=True)
        )
        self.cls1 = nn.Linear(num_classes,2)

        self.fc2 = nn.Sequential(
            nn.Linear(128 * block.expansion, num_classes*4),
            nn.Dropout(0.5),
            nn.ReLU(inplace=True),
            nn.Linear(num_classes*4, num_classes),
            nn.ReLU(inplace=True)
        )
        self.cls2 = nn.Linear(num_classes,2)
           
        self.fc3 = nn.Sequential(
            nn.Linear(256 * block.expansion, num_classes*4),
            nn.Dropout(0.5),
            nn.ReLU(inplace=True),
            nn.Linear(num_classes*4, num_classes),
            nn.ReLU(inplace=True)
        )
        self.cls3 = nn.Linear(num_classes,2)
        ...
        self.cls = nn.Linear(num_classes,2)
...
if __name__ == '__main__':
    ...
    # model = res2net50(num_classes=512)
